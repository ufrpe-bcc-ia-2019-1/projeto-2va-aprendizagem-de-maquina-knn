...
def obterVizinhos2(conjuntoTreino, conjuntoTeste, k):	#Função que retorna os k-vizinhos mais similares do conjunto de treinamento para uma determinada instância de teste
    distancias = []                             	#Calcula a distância para todas as instâncias e seleciona um subconjunto com os menores valores de distância
    length = len(conjuntoTeste) - 1                #Função da distância Manhantam
    for x in range(len(conjuntoTreino)):
        dist = distanciaManhattan(conjuntoTeste, conjuntoTreino[x], length)
        distancias.append((conjuntoTreino[x], dist))
...
def obterVizinhos3(conjuntoTreino, conjuntoTeste, k):	#Função que retorna os k-vizinhos mais similares do conjunto de treinamento para uma determinada instância de teste
    distancias = []       #Calcula a distância para todas as instâncias e seleciona um subconjunto com os menores valores de distância
    length = len(conjuntoTeste) - 1                #Função da distância Bray-curts
    for x in range(len(conjuntoTreino)):
        dist = distanciaBrayCurtis(conjuntoTeste, conjuntoTreino[x], length)
        distancias.append((conjuntoTreino[x], dist))
    distancias.sort(key=operator.itemgetter(1))
    vizinhos = []
    for x in range(k):
